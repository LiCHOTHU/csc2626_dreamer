...
class SlotFreewayConfig_1slot():
    ...
    params = './savi_params/savi_freeway_params-1slot'
    ...
    #latent space desc
    assert params.slot_size == 128*2 and params.num_slots == 1, 'Please update the RSSM params'
    rssm_type: str = 'discrete'
    embedding_size: int = 128*2
    ...
    rssm_info: Dict = field(default_factory=lambda:{'deter_size':128*2, 'stoch_size':128, 'class_size':2, 'category_size':128, 'min_std':0.1})
...
class SlotAsterixConfig_1slot():
    ...
    # SAVi config file
    cur_dir = os.path.dirname(os.path.realpath(__file__))
    params = './savi_params/savi_asterix_params-1slot'
    params = os.path.join(cur_dir, params)
    sys.path.append(os.path.dirname(params))
    params = importlib.import_module(os.path.basename(params))
    params = params.SlotAttentionParams()
