...
class SlotEvaluator(object):
    ...
    def load_model(self, config, model_path):
        ...
        if config.pixel:
            self.ObsEncoder = SlotObsEncoder(config.params).to(self.device).eval()
            self.ObsDecoder = SlotObsDecoder(config.params).to(self.device).eval()
        else:
            raise NotImplementedError('only pixel envs are supported for now')
