...
from dreamerv2.models.slot_pixel import SlotObsEncoder, SlotObsDecoder
...
class SlotTrainer(object):
    ...
    def representation_loss(self, obs, actions, rewards, nonterms):
        ...
        obs_loss = self._obs_loss(obs_dist, embed[:-1])
    ...
    def _model_initialize(self, config):
        ...
        if config.pixel:
            self.ObsEncoder = SlotObsEncoder(config.params).to(self.device)
            self.ObsDecoder = SlotObsDecoder(config.params).to(self.device)
        else:
            raise NotImplementedError('Only pixel observations are supported.')
