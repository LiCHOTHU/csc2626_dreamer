...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        'vision_size': (64, 64),  # Size (width, height) of vision observation; gets flipped internally to (rows, cols) format
    ...
    def build_observation_space(self):
        ...
        width, height = self.vision_size
        self.observation_space = gym.spaces.MultiBinary((3, height, width))
    ...
    def obs_vision(self):
        ...
        vision = self.render(mode='rgb_array', camera_id=1, width=width, height=height)
        vision = np.flip(vision)
        vision = vision[:, :, ::-1]
        image = np.array(vision, dtype='float32') / 255
